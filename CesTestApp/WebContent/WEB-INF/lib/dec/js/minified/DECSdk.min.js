var userAgent=navigator.userAgent,Availability={available:0,readonly:1,not_supported:2,not_supported_yet:3,not_supported_security:4,not_supported_policy:5,not_supported_other:6},DecErrorCodes={invalid_parameter:{errorcode:"invalid_parameter",errormessage:"Invalid parameter"},not_authenticated:{errorcode:"not_authenticated",errormessage:"Not authenticated"},not_authorized:{errorcode:"not_authorized",errormessage:"Not authorized"},connection_timeout:{errorcode:"connection_timeout",errormessage:"Communication error"},
not_subscribed:{errorcode:"not_subscribed",errormessage:"Not subscribed"},not_supported:{errorcode:"not_supported",errormessage:"Not Supported"},component_not_found:{errorcode:"component_not_found",errormessage:"Component not found"},span_not_found:{errorcode:"span_server_not_connected",errormessage:"Not connected to span server, Please check the configuration"}},EventType={ALL:"all",CREATE:"create",ADD:"add",UPDATE:"update",GET:"get",DELETE:"delete"},componentsArray="alertmanager appmanager calendar commerce connectivity identity media navigation notification policy sa search settings sms speech va vehicleinfo".split(" "),
ws=null,connectionType="";console.log("User agent :: ",userAgent);var pattern=userAgent.match(/Android/),version=null;null!=pattern&&0<pattern.length&&null!=pattern[0]&&0<pattern[0].length&&(version=pattern[0]);console.log("version",version);null!=version&&"Android"==version?(console.log("Going to use MB Bridge"),connectionType="mbbridge"):(console.log("Going to use SPAN"),connectionType="span");console.log("Using Connection Type: ",connectionType);
...
console.log("Notifying Systems"),notifySystems(c)):console.log("Is already subscribed ::: true"));return d};this.unsubscribe=function(a){var b=a;null==a&&(b=createSubscriptionHandle(a));storage[b]=null;console.log("Unsubscribed...");this.availableStatus=Availability.not_supported};this.get=function(a){if(this.availableStatus===Availability.not_supported)return console.log("Not Supported!"),new Promise(function(a,b){var c=JSON.stringify(DecErrorCodes.not_supported);b(Error(c))});var b=a||null,c="";
console.log("Get Called for NodeLevel \x3d "+this.nodeLevel+", leaf \x3d "+this.isLeaf());if(b)if(this.isLeaf())c=localStorage.getItem(this.nodeLevel);else{var d=Object.keys(localStorage),e=localStorage.getItem(this.nodeLevel),e=JSON.parse(e);if(this.isArray){console.log("JSON: "+a);var b=JSON.stringify(a),f=JSON.parse(b);console.log("json parse"+f);a=[];var h=[],g=0;for(attrKey in f)a[g]=attrKey,h[g]=f[attrKey],g++;for(var k=[],l=f=0;l<e.length;l++){for(var m=!0,b=0;b<g;b++)if(d=h[b],e[l][a[b]]!==
d){m=!1;break}m&&(k[f]=JSON.stringify(e[l]),f++)}console.log("GET Array Result: "+k);return new Promise(function(a,b){a(k)})}if(this.isMap){f=JSON.parse(a);a={};for(b=0;b<f.length;b++)d=f[b],null!==e[d]&&(a[d]=e[d]);c=JSON.stringify(a)}else{c="{";if(b instanceof Array)for(d=e=0;d<b.length;d++)0!==e&&(c+=","),c+="'"+this.nodeLevel+"."+b[d]+"':'"+localStorage.getItem(this.nodeLevel+"."+b[d])+"'",e++;else for(b=e=0;b<d.length;b++)isNaN(parseInt(d[b]))&&-1!=d[b].indexOf(this.nodeLevel)&&(0!==e&&(c+=","),
c+=d[b]+":'"+localStorage.getItem(d[b])+"'",e++);c+="}"}console.log("GET Result: "+c);console.log("GET Result: "+c.length);return new Promise(function(a,b){a(c)})}else c=getValueForHandle(this.isLeaf(),this.isArray,this.isMap,this.nodeLevel);console.log("Retreived JSON: "+c);return new Promise(function(a,b){a(c)})};this.set=function(a,b){var c=b||null;if(null===(a||null))return console.log("Set called without options!"),new Promise(function(a,b){var c=JSON.stringify(DecErrorCodes.invalid_parameter);
b(Error(c))});try{console.log("JSON: "+a);console.log("NodeLevel: "+this.nodeLevel);var d=null,d=null==c?EventType.ADD:EventType.UPDATE;if(!this.isLeaf()||this.isArray||this.isMap)if(this.isArray){console.log("Set called for array types");setItemsOfArrayType(this.nodeLevel,a,c);f=null;if(a instanceof Array)f=a;else{var e=[];e[0]=a;f=e}m={component:getComponent(this.nodeLevel),namespace:getNameSpace(this.nodeLevel),method:"set",eventType:d,value:f,options:c};console.log("Executing Subscription callback method");
executeSubscriptionCallbackForParentChild(this.nodeLevel,f,d);console.log("Notifying Systems");notifySystems(m)}else if(this.isMap)console.log("Set called at parent level for map type"),setItemsOfMapType(this.nodelLevel,a,c),m={component:getComponent(this.nodeLevel),namespace:getNameSpace(this.nodeLevel),method:"set",eventType:d,value:a,options:c},console.log("Executing Subscription callback method"),executeSubscriptionCallbackForParentChild(this.nodeLevel,a,d),console.log("Notifying Systems"),notifySystems(m);
else{console.log("Set called at parent level for non-array types");var f=!1,h=JSON.stringify(a),g=JSON.parse(h);console.log("json parse"+g);for(l in g)attrVal=g[l],console.log("ATTRIBUTE KEY::  "+l),console.log("ATTRIBUTE VALUE::  "+attrVal),this[l].isArray?(e=setItemsOfArrayType(this.nodeLevel+"."+l,attrVal,c),null!=e&&(f=!0)):f=setItemsOfLeafOrNonLeafType(this,l,attrVal);f&&(k=this.nodeLevel,-1==k.indexOf(".")&&(k+="."),m={component:getComponent(k),namespace:getNameSpace(k),method:"set",eventType:d,
value:a,options:c},console.log("Executing Subscription callback method"),executeSubscriptionCallbackForParentChild(this.nodeLevel,a,d),console.log("Notifying Systems"),notifySystems(m))}else{console.log("Set called at leaf level");localStorage.setItem(this.nodeLevel,a);var k=this.nodeLevel,l=k.substring(k.lastIndexOf(".")+1),f={};f[l]=a;var m={component:getComponent(k),namespace:k.substring(k.indexOf(".")+1,k.lastIndexOf(".")),method:"set",eventType:d,value:f,options:c};console.log("Executing Subscription callback method");
var p=k.substring(0,k.lastIndexOf("."));executeSubscriptionCallbackForParentChild(p,f,d);console.log("Notifying Systems");notifySystems(m)}}catch(n){return console.log("Exception: "+n.message),new Promise(function(a,b){b(Error("Error: "+n.message))})}return new Promise(function(a,b){a("success")})};this.delete=function(a){a=a||null;console.log("Delete Called for NodeLevel \x3d "+this.nodeLevel+", leaf \x3d "+this.isLeaf());a={component:getComponent(this.nodeLevel),namespace:getNameSpace(this.nodeLevel),
method:"clear",eventType:EventType.DELETE,options:a};console.log("Notifying Systems");notifySystems(a);return new Promise(function(a,c){a("Successfully removed")})}};
...
function initialize(a,b,c){console.log("Components passed ::: ",b);if(null==c)throw console.log("Cannot Initialize the component as Application Id is not provided."),"Cannot Initialize the component as Application Id is not provided.";var d=checkForComponentsValidity(b);if(null!=d)console.log("Cannot Initialize... "+d+" is not a valid component"),b=new DecError(DecErrorCodes.component_not_found.errorcode,DecErrorCodes.component_not_found.errormessage),null!=a&&a(b);else{drive.callback=a;for(d=0;d<
b.length;d++){var e=b[d];console.log("Initializing "+e);drive[e]=new DecCommon;drive[e].setLeafStatus(!1);drive[e].setNodeLevel(b[d]);drive[e].setAvailableStatus(Availability.available);null!=c&&drive[e].setAppId(c);var f=eval("("+e+")");initializeComponents(drive[e],f)}"span"==connectionType?(console.log("Connection Type is Span. Initializing Web Socket"),RunSpanMessageBroker()):(console.log("Connection Type is MB bridge"),b=new DecSuccess("0","Successfully Initialized the components"),null!=a&&
a(b))}}function showLocalStorage(){var a="",b="\x3ctable\x3e\x3ctr\x3e\x3cth\x3eName\x3c/th\x3e\x3cth\x3eValue\x3c/th\x3e\x3c/tr\x3e",c=0;console.log("Local Storage length is"+localStorage.length);for(c=0;c<=localStorage.length-1;c++)a=localStorage.key(c),b+="\x3ctr\x3e\x3ctd\x3e"+a+"\x3c/td\x3e\x3ctd\x3e"+localStorage.getItem(a)+"\x3c/td\x3e\x3c/tr\x3e";document.write(b+"\x3c/table\x3e")}var Operation={READ:"read",WRITE:"write",PRIVATE:"private",POLICY:"policy"};
...
encodedData=encodeURI(a);d={"dec.SetRequest":{sender:{sender:d,receivers:["DECCore"],data:encodedData}}};console.log(JSON.stringify(d));console.log("Sending msg is ::: "+JSON.stringify(d));ws.send(JSON.stringify(d))}function getApplicationId(a){try{if(null!=drive[a])return drive[a].getAppId()}catch(b){}return"DECJSSDK"}function createSubscriptionHandle(a){return getHashCode(a)+a}function createDataHandle(a,b){return getHashCode(a+"."+b)}
function createDataHandleSubscription(a,b,c){return getHashCode(a+"."+b+"."+c)}function initializeComponents(a,b){for(var c in b){a[c]=new DecCommon;var d=a.getNodeLevel();a[c].setNodeLevel(""+d+"."+c);a[c].getNodeLevel();"java.util.Map"===b[c].type?a[c].isMap=!0:"java.util.List"===b[c].type&&(a[c].isArray=!0);null!=b[c].properties?(a[c].setLeafStatus(!1),initializeComponents(a[c],b[c].properties)):a[c].setLeafStatus(!0)}}
...
a);console.log("index of letrequest is::"+a.indexOf("dec.LetRequest"));if(-1!=a.indexOf("dec.LetRequest")){a=JSON.parse(a)["dec.LetRequest"];decodedData=decodeURIComponent(a.request.data);console.log("DecodedData::::"+decodedData);var b=JSON.parse(decodedData),c=b.component,d=b.nameSpace;a=b.value;var e=b.options,f=b.method,b=b.eventType;console.log("component is:::: "+c);console.log("namespace is:::: "+d);console.log("value is:::: "+a);console.log("options is:::: "+e);console.log("method is:::: "+
f);console.log("eventType is:::: "+b);var h="",h=null==d||0==d.length?c:c+"."+d,g=getCurrentObject(c,d);if(null!=g)if(null!=f&&"clear"===f)c=e||null,console.log("Received Delete method call for NodeLevel \x3d "+h+", leaf \x3d "+g.isLeaf()),d=null,d=c?handleDeleteWithOptions(h,e,g.isLeaf(),g.isArray,g.isMap):handleDeleteWithNoOptions(h,g.isLeaf(),g.isArray,g.isMap),console.log("Deleted Item ::: ",d),null==a&&(a=d);else if(g.isArray&&null!=a)console.log("Received component is of Array Type"),setItemsOfArrayType(h,
a,e);else for(attrKey in console.log("Received component is of non Array Type"),a){attrVal=a[attrKey];console.log("ATTRIBUTE KEY::  "+attrKey);console.log("ATTRIBUTE VALUE::  "+attrVal);var f="",f=null==d||""==d?attrKey:d+"."+attrKey,k=getCurrentObject(c,f);null!=k?k.isArray?setItemsOfArrayType(h+"."+attrKey,attrVal,e):setItemsOfLeafOrNonLeafType(g,attrKey,attrVal):console.log("Unable to get the current object for "+c+"."+f)}executeSubscriptionCallbackForParentChild(h,a,b)}else console.log("Received Set dec request..ignoring...")},
ws.onclose=function(){console.log("Connection is closed...")},ws.onerror=function(a){console.log("websocket connection to host "+properties.host+" port "+properties.port+" failed");console.log("error is::"+a);a=new DecError(DecErrorCodes.span_not_found.errorcode,DecErrorCodes.span_not_found.errormessage);var b=drive.callback;null!=b&&eval("("+b+")").call(null,a)}):console.log("WebSocket NOT supported by your Browser!")}"mbbridge"==connectionType&&mb.setReceiveCallback("receiveMessage");
function sendMessage(a,b,c){b=JSON.stringify(b);mb.sendMessage(a,b,c)}
